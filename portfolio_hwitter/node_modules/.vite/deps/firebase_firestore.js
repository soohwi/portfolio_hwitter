...
var Timestamp = class _Timestamp {
   ...
   */
  constructor(e, t) {
    if (this.seconds = e, this.nanoseconds = t, t < 0) throw new FirestoreError(v2.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + t);
    if (t >= 1e9) throw new FirestoreError(v2.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + t);
    if (e < -62135596800) throw new FirestoreError(v2.INVALID_ARGUMENT, "Timestamp seconds out of range: " + e);
    if (e >= 253402300800) throw new FirestoreError(v2.INVALID_ARGUMENT, "Timestamp seconds out of range: " + e);
  }
  /**
...
var ResourcePath = class _ResourcePath extends BasePath {
  ...
  static fromString(...e) {
    const t = [];
    for (const n of e) {
      if (n.indexOf("//") >= 0) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid segment (${n}). Paths must not contain // in them.`);
...
var FieldPath$1 = class _FieldPath$1 extends BasePath {
  ...
  static fromServerFormat(e) {
    ...
    const __PRIVATE_addCurrentSegment = () => {
      if (0 === n.length) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid field path (${e}). Paths must not be empty, begin with '.', end with '.', or contain '..'`);
      t.push(n), n = "";
    };
    let i = false;
    for (; r2 < e.length; ) {
      const t2 = e[r2];
      if ("\\" === t2) {
        if (r2 + 1 === e.length) throw new FirestoreError(v2.INVALID_ARGUMENT, "Path has trailing escape character: " + e);
        const t3 = e[r2 + 1];
        if ("\\" !== t3 && "." !== t3 && "`" !== t3) throw new FirestoreError(v2.INVALID_ARGUMENT, "Path has invalid escape sequence: " + e);
        n += t3, r2 += 2;
      } else "`" === t2 ? (i = !i, r2++) : "." !== t2 || i ? (n += t2, r2++) : (__PRIVATE_addCurrentSegment(), r2++);
    }
    if (__PRIVATE_addCurrentSegment(), i) throw new FirestoreError(v2.INVALID_ARGUMENT, "Unterminated ` in path: " + e);
...
async function __PRIVATE_ignoreIfPrimaryLeaseLoss(e) {
  if (e.code !== v2.FAILED_PRECONDITION || e.message !== F2) throw e;
...
var __PRIVATE_IndexBackfiller = class {
  ...
  re(e, t) {
    ...
    return PersistencePromise.doWhile(() => true === i && r2 > 0, () => this.localStore.indexManager.getNextCollectionGroupToUpdate(e).next((t2) => {
      if (null !== t2 && !n.has(t2)) return __PRIVATE_logDebug("IndexBackiller", `Processing collection: ${t2}`), this.ie(e, t2, r2).next((e2) => {
        r2 -= e2, n.add(t2);
      });
      i = false;
    })).next(() => t - r2);
...
function __PRIVATE_queryOrderBy(e) {
  ...
  if (null === t.Pe) {
    ...
    if (null !== e2 && null === n)
      ...
      let e3 = false;
      for (const n2 of t.explicitOrderBy) t.Pe.push(n2), n2.field.isKeyField() && (e3 = true);
      if (!e3) {
...
function __PRIVATE_queryToTarget(e) {
  const t = __PRIVATE_debugCast(e);
  if (!t.Ie) if ("F" === t.limitType) t.Ie = __PRIVATE_newTarget(t.path, t.collectionGroup, __PRIVATE_queryOrderBy(t), t.filters, t.limit, t.startAt, t.endAt);
  else {
    const e2 = [];
...
function __PRIVATE_queryMatches(e, t) {
  return t.isFoundDocument() && function __PRIVATE_queryMatchesPathAndCollectionGroup(e2, t2) {
    ...
    */
    ...
    }(e2.startAt, __PRIVATE_queryOrderBy(e2), t2)) return false;
    if (e2.endAt && !function __PRIVATE_boundSortsAfterDocument(e3, t3, n) {
      const r2 = __PRIVATE_boundCompareToDocument(e3, t3, n);
      return e3.inclusive ? r2 >= 0 : r2 > 0;
    }(e2.endAt, __PRIVATE_queryOrderBy(e2), t2)) return false;
    return true;
  }(e, t);
}
...
var __PRIVATE_WatchChangeAggregator = class {
  ...
  tt(e, t) {
    ...
    try {
      ...
      if (e2 instanceof __PRIVATE_Base64DecodeError) return __PRIVATE_logWarn("Decoding the base64 bloom filter in existence filter failed (" + e2.message + "); ignoring the bloom filter and falling back to full re-query."), {
        status: 1
        /* BloomFilterApplicationStatus.Skipped */
      };
      throw e2;
...
function fromName(e, t) {
  ...
  if (n.get(1) !== e.databaseId.projectId) throw new FirestoreError(v2.INVALID_ARGUMENT, "Tried to deserialize key from different project: " + n.get(1) + " vs " + e.databaseId.projectId);
  if (n.get(3) !== e.databaseId.database) throw new FirestoreError(v2.INVALID_ARGUMENT, "Tried to deserialize key from different database: " + n.get(3) + " vs " + e.databaseId.database);
  ...
  en(e) {
    ...
    if (void 0 !== this.Xt) {
      if (!r2.has(this.Xt.field.canonicalString())) {
        const e2 = n[i];
        if (!this.nn(this.Xt, e2) || !this.rn(this.Yt[s++], e2)) return false;
      }
      ++i;
    }
    for (; i < n.length; ++i) {
...
var __PRIVATE_IndexedDbIndexManager = class {
  ...
  ln(e, t, n) {
    ...
    for (const s of __PRIVATE_fieldIndexGetDirectionalSegments(e)) {
      const e2 = n[i++];
      for (const n2 of r2) if (this.An(t, s.fieldPath) && isArray(e2)) r2 = this.Rn(r2, s, e2);
      else {
  ...
  gn(e, t) {
    ...
    if (null != i) {
      ...
      if (isArray(s)) for (const i2 of s.arrayValue.values || []) n = n.add(new __PRIVATE_IndexEntry(t.indexId, e.key, this.Pn(i2), r2));
    } else n = n.add(new __PRIVATE_IndexEntry(t.indexId, e.key, Ee, r2));
    return n;
  }
  /**
  ...
  createRange(e, t, n) {
    ...
    for (let e2 = 0; e2 < r2.length; e2 += 2) {
      if (this.Sn(r2[e2], r2[e2 + 1])) return [];
...
var __PRIVATE_MemoryMutationQueue = class {
  ...
  addMutationBatch(e, t, n, r2) {
    ...
    for (const t2 of r2) this.pr = this.pr.add(new __PRIVATE_DocReference(t2.key, i)), this.indexManager.addToCollectionParentIndex(e, t2.key.path.popLast());
...
var __PRIVATE_IndexedDbPersistence = class ___PRIVATE_IndexedDbPersistence {
  constructor(e, t, n, r2, i, s, o, _, a, u, c = 15) {
    ...
    this.hi = (e2) => Promise.resolve(), !___PRIVATE_IndexedDbPersistence.v()) throw new FirestoreError(v2.UNIMPLEMENTED, "This platform is either missing IndexedDB or is known to have an incomplete implementation. Offline persistence has been disabled.");
  ...
  Ei() {
    return this.runTransaction("updateClientMetadataAndTryBecomePrimary", "readwrite", (e) => __PRIVATE_clientMetadataStore(e).put({
      ...
      if (this.isPrimary) return this.mi(e).next((e2) => {
   ...
   */
    ...
    return __PRIVATE_primaryClientStore(e).get("owner").next((t) => {
      if (null !== t && this.bi(t.leaseTimestampMs, 5e3) && !this.Ci(t.ownerId)) {
        if (this.yi(t) && this.networkEnabled) return true;
        if (!this.yi(t)) {
  ...
  runTransaction(e, t, n) {
    ...
    return this.Ii.runTransaction(e, r2, i, (r3) => (s = new __PRIVATE_IndexedDbTransaction(r3, this.Br ? this.Br.next() : __PRIVATE_ListenSequence.ae), "readwrite-primary" === t ? this.mi(s).next((e2) => !!e2 || this.fi(s)).next((t2) => {
      if (!t2) throw __PRIVATE_logError(`Failed to obtain primary lease for action '${e}'.`), this.isPrimary = false, this.ii.enqueueRetryable(() => this.hi(false)), new FirestoreError(v2.FAILED_PRECONDITION, F2);
      return n(s);
  ...
  Bi(e) {
    return __PRIVATE_primaryClientStore(e).get("owner").next((e2) => {
      if (null !== e2 && this.bi(e2.leaseTimestampMs, 5e3) && !this.Ci(e2.ownerId) && !this.yi(e2) && !(this.oi || this.allowTabSynchronization && e2.allowTabSynchronization)) throw new FirestoreError(v2.FAILED_PRECONDITION, Ae);
    });
  }
  /**
...
function __PRIVATE_localStoreExecuteQuery(e, t, n) {
  ...
  return r2.persistence.runTransaction("Execute query", "readonly", (e2) => function __PRIVATE_localStoreGetTargetData(e3, t2, n2) {
    ...
    if (t2) return i = t2.lastLimboFreeSnapshotVersion, r2.kr.getMatchingKeysForTargetId(e2, t2.targetId).next((e3) => {
      s = e3;
    });
...
async function __PRIVATE_localStoreSaveNamedQuery(e, t, n = __PRIVATE_documentKeySet()) {
  ...
  return i.persistence.runTransaction("Save named query", "readwrite", (e2) => {
    ...
    if (r2.snapshotVersion.compareTo(s) >= 0) return i.Qr.saveNamedQuery(e2, t);
...
var __PRIVATE_WebStorageSharedClientState = class {
  ...
  async start() {
    ...
    for (const e2 of this.As) this.Es(e2);
  ...
  Es(e) {
    ...
    if (t.storageArea === this.storage) {
      if (__PRIVATE_logDebug("SharedClientState", "EVENT", t.key, t.newValue), t.key === this.Rs) return void __PRIVATE_logError("Received WebStorage notification for local change. Another client might have garbage-collected our state");
          ...
          if (null !== t.key) {
            if (this.fs.test(t.key)) {
              ...
              }
              {
                ...
                if (e2) return this.Ls(e2.clientId, e2);
              }
  ...
  async Qs(e) {
    if (e.user.uid === this.currentUser.uid) return this.syncEngine.Gs(e.batchId, e.state, e.error);
          ...
          switch (_.getLastErrorCode()) {
            ...
            case ErrorCode.HTTP_ERROR:
              ...
              if (__PRIVATE_logDebug(me, `RPC '${e}' ${i} failed with status:`, n2, "response text:", _.getResponseText()), n2 > 0) {
                ...
                } else o(new FirestoreError(v2.UNKNOWN, "Server responded with status " + _.getStatus()));
              } else
  ...
  wo(e, t, n) {
    ...
    return __PRIVATE_unguardedEventListen(c, WebChannel.EventType.OPEN, () => {
      ...
      if (!h) {
        ...
        } else __PRIVATE_logDebug(me, `RPC '${e}' stream ${r2} received:`, i2), P2.Io(i2);
      }
...
var __PRIVATE_DatastoreImpl = class extends class Datastore {
  ...
  P_() {
    if (this.h_) throw new FirestoreError(v2.FAILED_PRECONDITION, "The client has already been terminated.");
  }
...
async function __PRIVATE_enableNetworkInternal(e) {
  if (__PRIVATE_canUseNetwork(e)) for (const t of e.w_) await t(
    /* enabled= */
    true
  );
}
async function __PRIVATE_disableNetworkInternal(e) {
  for (const t of e.w_) await t(
    /* enabled= */
    false
  );
}
...
async function __PRIVATE_onWatchStreamChange(e, t, n) {
  ...
  else if (t instanceof __PRIVATE_DocumentWatchChange ? e.D_.We(t) : t instanceof __PRIVATE_ExistenceFilterChange ? e.D_.Ze(t) : e.D_.je(t), !n.isEqual(SnapshotVersion.min())) try {
...
function __PRIVATE_wrapInUserErrorIfRecoverable(e, t) {
  if (__PRIVATE_logError("AsyncQueue", `${t}: ${e}`), __PRIVATE_isIndexedDbTransactionError(e)) return new FirestoreError(v2.UNAVAILABLE, `${t}: ${e}`);
  throw e;
}
...
function __PRIVATE_eventManagerOnWatchChange(e, t) {
  ...
  for (const e2 of t) {
    ...
    if (i) {
      for (const t3 of i.listeners) t3.B_(e2) && (r2 = true);
      i.x_ = e2;
    }
  }
...
var __PRIVATE_BundleLoader = class {
  ...
  H_(e) {
    ...
    } else e.G_.document && (this.documents[this.documents.length - 1].document = e.G_.document, ++t);
...
async function __PRIVATE_syncEngineUnlisten(e, t) {
  ...
  if (i.length > 1) return n.da.set(r2.targetId, i.filter((e2) => !__PRIVATE_queryEquals(e2, t))), void n.Ea.delete(t);
...
function __PRIVATE_removeAndCleanupTarget(e, t, n = null) {
  ...
  for (const r2 of e.da.get(t)) e.Ea.delete(r2), n && e.Ta.Sa(r2, n);
...
function __PRIVATE_updateTrackedLimbos(e, t, n) {
  for (const r2 of n) if (r2 instanceof __PRIVATE_AddedLimboDocument) e.ma.addReference(r2.key, t), __PRIVATE_trackLimboChange(e, r2);
...
function __PRIVATE_syncEngineGetRemoteKeysForTarget(e, t) {
  ...
  if (r2 && r2.Ia) return __PRIVATE_documentKeySet().add(r2.key);
  {
    let e2 = __PRIVATE_documentKeySet();
    const r3 = n.da.get(t);
    if (!r3) return e2;
...
var __PRIVATE_MultiTabOfflineComponentProvider = class extends __PRIVATE_IndexedDbOfflineComponentProvider {
  ...
  createSharedClientState(e) {
    ...
    if (!__PRIVATE_WebStorageSharedClientState.v(t)) throw new FirestoreError(v2.UNIMPLEMENTED, "IndexedDB persistence is only available on platforms that support LocalStorage.");
...
var Transaction$2 = class {
  ...
  async lookup(e) {
    if (this.ensureCommitNotCalled(), this.mutations.length > 0) throw new FirestoreError(v2.INVALID_ARGUMENT, "Firestore transactions require all reads to be executed before all writes.");
...
var FirestoreClient = class {
   ...
   */
  verifyNotTerminated() {
    if (this.asyncQueue.isShuttingDown) throw new FirestoreError(v2.FAILED_PRECONDITION, "The client has already been terminated.");
  }
  terminate() {
...
function __PRIVATE_validateNonEmptyArgument(e, t, n) {
  if (!n) throw new FirestoreError(v2.INVALID_ARGUMENT, `Function ${e}() cannot be called with an empty ${t}.`);
}
function __PRIVATE_validateIsNotUsedTogether(e, t, n, r2) {
  if (true === t && true === r2) throw new FirestoreError(v2.INVALID_ARGUMENT, `${e} and ${n} cannot be used together.`);
}
function __PRIVATE_validateDocumentPath(e) {
  if (!DocumentKey.isDocumentKey(e)) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid document reference. Document references must have an even number of segments, but ${e} has ${e.length}.`);
}
function __PRIVATE_validateCollectionPath(e) {
  if (DocumentKey.isDocumentKey(e)) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid collection reference. Collection references must have an odd number of segments, but ${e} has ${e.length}.`);
}
...
function __PRIVATE_cast(e, t) {
  ...
  (e = e._delegate), !(e instanceof t)) {
    if (t.name === e.constructor.name) throw new FirestoreError(v2.INVALID_ARGUMENT, "Type does not match the expected instance. Did you pass a reference from a different Firestore SDK?");
    {
...
function __PRIVATE_validatePositiveNumber(e, t) {
  if (t <= 0) throw new FirestoreError(v2.INVALID_ARGUMENT, `Function ${e}() requires a positive number, but it was: ${t}.`);
}
var FirestoreSettingsImpl = class {
  constructor(e) {
    var t, n;
    if (void 0 === e.host) {
      if (void 0 !== e.ssl) throw new FirestoreError(v2.INVALID_ARGUMENT, "Can't provide ssl option if host option is not set");
    ...
    if (this.credentials = e.credentials, this.ignoreUndefinedProperties = !!e.ignoreUndefinedProperties, this.localCache = e.localCache, void 0 === e.cacheSizeBytes) this.cacheSizeBytes = 41943040;
      ...
      if (-1 !== e.cacheSizeBytes && e.cacheSizeBytes < 1048576) throw new FirestoreError(v2.INVALID_ARGUMENT, "cacheSizeBytes must be at least 1048576");
    ...
    __PRIVATE_validateIsNotUsedTogether("experimentalForceLongPolling", e.experimentalForceLongPolling, "experimentalAutoDetectLongPolling", e.experimentalAutoDetectLongPolling), this.experimentalForceLongPolling = !!e.experimentalForceLongPolling, this.experimentalForceLongPolling ? this.experimentalAutoDetectLongPolling = false : void 0 === e.experimentalAutoDetectLongPolling ? this.experimentalAutoDetectLongPolling = true : (
      ...
      if (void 0 !== e2.timeoutSeconds) {
        if (isNaN(e2.timeoutSeconds)) throw new FirestoreError(v2.INVALID_ARGUMENT, `invalid long polling timeout: ${e2.timeoutSeconds} (must not be NaN)`);
        if (e2.timeoutSeconds < 5) throw new FirestoreError(v2.INVALID_ARGUMENT, `invalid long polling timeout: ${e2.timeoutSeconds} (minimum allowed value is 5)`);
        if (e2.timeoutSeconds > 30) throw new FirestoreError(v2.INVALID_ARGUMENT, `invalid long polling timeout: ${e2.timeoutSeconds} (maximum allowed value is 30)`);
      }
...
var Firestore$1 = class {
   ...
   */
  get app() {
    if (!this._app) throw new FirestoreError(v2.FAILED_PRECONDITION, "Firestore was not initialized using the Firebase SDK. 'app' is not available");
    return this._app;
  }
  get _initialized() {
  ...
  _setSettings(e) {
    if (this._settingsFrozen) throw new FirestoreError(v2.FAILED_PRECONDITION, "Firestore has already been started and its settings can no longer be changed. You can only modify settings before calling any other methods on a Firestore object.");
...
function connectFirestoreEmulator(e, t, n, r2 = {}) {
  ...
  if ("firestore.googleapis.com" !== s.host && s.host !== o && __PRIVATE_logWarn("Host has been set in both settings() and connectFirestoreEmulator(), emulator host will be used."), e._setSettings(Object.assign(Object.assign({}, s), {
    ...
    if ("string" == typeof r2.mockUserToken) t2 = r2.mockUserToken, n2 = User.MOCK_USER;
      ...
      if (!s2) throw new FirestoreError(v2.INVALID_ARGUMENT, "mockUserToken must contain 'sub' or 'user_id' field!");
      n2 = new User(s2);
    }
...
function collection(e, t, ...n) {
  ...
  {
    if (!(e instanceof DocumentReference || e instanceof CollectionReference)) throw new FirestoreError(v2.INVALID_ARGUMENT, "Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore");
...
function collectionGroup(e, t) {
  if (e = __PRIVATE_cast(e, Firestore$1), __PRIVATE_validateNonEmptyArgument("collectionGroup", "collection id", t), t.indexOf("/") >= 0) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid collection ID '${t}' passed to function collectionGroup(). Collection IDs must not contain '/'.`);
  return new Query(
    e,
    /* converter= */
    null,
...
var __PRIVATE_AsyncQueueImpl = class {
   ...
   */
  su(e) {
    for (const t of this.ja) if (t.timerId === e) return true;
    return false;
  }
  /**
  ...
  ou(e) {
    return this.iu().then(() => {
      ...
      for (const t of this.ja) if (t.skipDelay(), "all" !== e && t.timerId === e) break;
      return this.iu();
    });
  }
  /**
...
function initializeFirestore(e, t, n) {
  ...
  if (void 0 !== t.cacheSizeBytes && void 0 !== t.localCache) throw new FirestoreError(v2.INVALID_ARGUMENT, "cache and cacheSizeBytes cannot be specified at the same time as cacheSizeBytes willbe deprecated. Instead, specify the cache size in the cache object");
  if (void 0 !== t.cacheSizeBytes && -1 !== t.cacheSizeBytes && t.cacheSizeBytes < 1048576) throw new FirestoreError(v2.INVALID_ARGUMENT, "cacheSizeBytes must be at least 1048576");
...
function enableIndexedDbPersistence(e, t) {
  ...
  if (n._uninitializedComponentsProvider) throw new FirestoreError(v2.FAILED_PRECONDITION, "SDK cache is already specified.");
...
function enableMultiTabIndexedDbPersistence(e) {
  ...
  if (t._uninitializedComponentsProvider) throw new FirestoreError(v2.FAILED_PRECONDITION, "SDK cache is already specified.");
...
function clearIndexedDbPersistence(e) {
  if (e._initialized && !e._terminated) throw new FirestoreError(v2.FAILED_PRECONDITION, "Persistence can only be cleared before a Firestore instance is initialized or after it is terminated.");
  const t = new __PRIVATE_Deferred();
...
function __PRIVATE_verifyNotInitialized(e) {
  if (e._initialized || e._terminated) throw new FirestoreError(v2.FAILED_PRECONDITION, "Firestore has already been started and persistence can no longer be enabled. You can only enable persistence before calling any other methods on a Firestore object.");
}
...
var FieldPath = class {
   ...
   */
  constructor(...e) {
    for (let t = 0; t < e.length; ++t) if (0 === e[t].length) throw new FirestoreError(v2.INVALID_ARGUMENT, "Invalid field name at argument $(i + 1). Field names must not be empty.");
...
var GeoPoint = class {
   ...
   */
  constructor(e, t) {
    if (!isFinite(e) || e < -90 || e > 90) throw new FirestoreError(v2.INVALID_ARGUMENT, "Latitude must be a number between -90 and 90, but was: " + e);
    if (!isFinite(t) || t < -180 || t > 180) throw new FirestoreError(v2.INVALID_ARGUMENT, "Longitude must be a number between -180 and 180, but was: " + t);
...
var __PRIVATE_ParseContextImpl = class ___PRIVATE_ParseContextImpl {
  ...
  Pu(e) {
    if (0 === e.length) throw this.Eu("Document fields must not be empty");
    if (__PRIVATE_isWrite(this.uu) && pe.test(e)) throw this.Eu('Document fields cannot begin and end with "__"');
  }
};
...
function __PRIVATE_parseSetData(e, t, n, r2, i, s = {}) {
  ...
  if (s.merge) a = new FieldMask(o.fieldMask), u = o.fieldTransforms;
    ...
    for (const r3 of s.mergeFields) {
      ...
      if (!o.contains(i2)) throw new FirestoreError(v2.INVALID_ARGUMENT, `Field '${i2}' is specified in your field mask but missing from your input data.`);
...
var __PRIVATE_DeleteFieldValueImpl = class ___PRIVATE_DeleteFieldValueImpl extends FieldValue {
  _toFieldTransform(e) {
    if (2 !== e.uu) throw 1 === e.uu ? e.Eu(`${this._methodName}() can only appear at the top level of your update data`) : e.Eu(`${this._methodName}() cannot be used with set() unless you pass {merge:true}`);
...
function __PRIVATE_parseUpdateVarargs(e, t, n, r2, i, s) {
  ...
  if (s.length % 2 != 0) throw new FirestoreError(v2.INVALID_ARGUMENT, `Function ${t}() needs to be called with an even number of arguments that alternate between field names and values.`);
...
function __PRIVATE_parseData(e, t) {
  ...
  if (e instanceof FieldValue)
    return function __PRIVATE_parseSentinelFieldValue(e2, t2) {
      if (!__PRIVATE_isWrite(t2.uu)) throw t2.Eu(`${e2._methodName}() can only be used with update() and set()`);
      if (!t2.path) throw t2.Eu(`${e2._methodName}() is not currently supported inside arrays`);
  ...
  if (
    ...
    if (t.settings.hu && 4 !== t.uu) throw t.Eu("Nested arrays are not supported");
  ...
  return function __PRIVATE_parseScalarValue(e2, t2) {
    ...
    if (e2 instanceof DocumentReference) {
      ...
      if (!r2.isEqual(n)) throw t2.Eu(`Document reference is for database ${r2.projectId}/${r2.database} but should be for database ${n.projectId}/${n.database}`);
      return {
...
function __PRIVATE_fieldPathFromDotSeparatedString(e, t, n) {
  if (t.search(ye) >= 0) throw __PRIVATE_createError(
...
function __PRIVATE_validateHasExplicitOrderByForLimitToLast(e) {
  if ("L" === e.limitType && 0 === e.explicitOrderBy.length) throw new FirestoreError(v2.UNIMPLEMENTED, "limitToLast() queries require specifying at least one orderBy() clause");
}
...
function query(e, t, ...n) {
  ...
  t instanceof AppliableConstraint && r2.push(t), r2 = r2.concat(n), function __PRIVATE_validateQueryConstraintArray(e2) {
    ...
    if (t2 > 1 || t2 > 0 && n2 > 0) throw new FirestoreError(v2.INVALID_ARGUMENT, "InvalidQuery. When using composite filters, you cannot use more than one filter at the top level. Consider nesting the multiple filters within an `and(...)` statement. For example: change `query(query, where(...), or(...))` to `query(query, and(where(...), or(...)))`.");
  }(r2);
...
var QueryFieldFilterConstraint = class _QueryFieldFilterConstraint extends QueryConstraint {
  ...
  _parse(e) {
    const t = __PRIVATE_newUserDataReader(e.firestore), n = function __PRIVATE_newQueryFilter(e2, t2, n2, r2, i, s, o) {
      let _;
      if (i.isKeyField()) {
        if ("array-contains" === s || "array-contains-any" === s) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid Query. You can't perform '${s}' queries on documentId().`);
        if ("in" === s || "not-in" === s) {
...
var QueryOrderByConstraint = class _QueryOrderByConstraint extends QueryConstraint {
  ...
  _apply(e) {
    const t = function __PRIVATE_newQueryOrderBy(e2, t2, n) {
      if (null !== e2.startAt) throw new FirestoreError(v2.INVALID_ARGUMENT, "Invalid query. You must not call startAt() or startAfter() before calling orderBy().");
      if (null !== e2.endAt) throw new FirestoreError(v2.INVALID_ARGUMENT, "Invalid query. You must not call endAt() or endBefore() before calling orderBy().");
...
function __PRIVATE_newQueryBoundFromDocOrFields(e, t, n, r2) {
  if (n[0] = getModularInstance(n[0]), n[0] instanceof DocumentSnapshot$1) return function __PRIVATE_newQueryBoundFromDocument(e2, t2, n2, r3, i) {
    if (!r3) throw new FirestoreError(v2.NOT_FOUND, `Can't use a DocumentSnapshot that doesn't exist for ${n2}().`);
    const s = [];
    for (const n3 of __PRIVATE_queryOrderBy(e2)) if (n3.field.isKeyField()) s.push(__PRIVATE_refValue(t2, r3.key));
    else {
      const e3 = r3.data.field(n3.field);
      if (__PRIVATE_isServerTimestamp(e3)) throw new FirestoreError(v2.INVALID_ARGUMENT, 'Invalid query. You are trying to start or end a query using a document for which the field "' + n3.field + '" is an uncommitted server timestamp. (Since the value of this field is unknown, you cannot start/end a query with it.)');
      if (null === e3) {
  ...
  }(e._query, e.firestore._databaseId, t, n[0]._document, r2);
  {
    ...
    return function __PRIVATE_newQueryBoundFromFields(e2, t2, n2, r3, i2, s) {
      ...
      if (i2.length > o.length) throw new FirestoreError(v2.INVALID_ARGUMENT, `Too many arguments provided to ${r3}(). The number of arguments must be less than or equal to the number of orderBy() clauses`);
      const _ = [];
      for (let s2 = 0; s2 < i2.length; s2++) {
        const a = i2[s2];
        if (o[s2].field.isKeyField()) {
          if ("string" != typeof a) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid query. Expected a string for document ID in ${r3}(), but got a ${typeof a}`);
          if (!__PRIVATE_isCollectionGroupQuery(e2) && -1 !== a.indexOf("/")) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid query. When querying a collection and ordering by documentId(), the value passed to ${r3}() must be a plain document ID, but '${a}' contains a slash.`);
          const n3 = e2.path.child(ResourcePath.fromString(a));
          if (!DocumentKey.isDocumentKey(n3)) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid query. When querying a collection group and ordering by documentId(), the value passed to ${r3}() must result in a valid document path, but '${n3}' is not because it contains an odd number of segments.`);
...
function __PRIVATE_parseDocumentIdValue(e, t, n) {
  if ("string" == typeof (n = getModularInstance(n))) {
    if ("" === n) throw new FirestoreError(v2.INVALID_ARGUMENT, "Invalid query. When querying with documentId(), you must provide a valid document ID, but it was an empty string.");
    if (!__PRIVATE_isCollectionGroupQuery(t) && -1 !== n.indexOf("/")) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid query. When querying a collection by documentId(), you must provide a plain document ID, but '${n}' contains a '/' character.`);
    const r2 = t.path.child(ResourcePath.fromString(n));
    if (!DocumentKey.isDocumentKey(r2)) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid query. When querying a collection group by documentId(), the value provided must result in a valid document path, but '${r2}' is not because it has an odd number of segments (${r2.length}).`);
...
function __PRIVATE_validateDisjunctiveFilterElements(e, t) {
  if (!Array.isArray(e) || 0 === e.length) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid Query. A non-empty array is required for '${t.toString()}' filters.`);
}
function __PRIVATE_validateNewFieldFilter(e, t) {
  if (t.isInequality()) {
    const n2 = __PRIVATE_getInequalityFilterField(e), r2 = t.field;
    if (null !== n2 && !n2.isEqual(r2)) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid query. All where filters with an inequality (<, <=, !=, not-in, >, or >=) must be on the same field. But you have inequality filters on '${n2.toString()}' and '${r2.toString()}'`);
...
function __PRIVATE_validateOrderByAndInequalityMatch(e, t, n) {
  if (!n.isEqual(t)) throw new FirestoreError(v2.INVALID_ARGUMENT, `Invalid query. You have a where filter with an inequality (<, <=, !=, not-in, >, or >=) on field '${t.toString()}' and so you must also use '${t.toString()}' as your first argument to orderBy(), but your first orderBy() is on field '${n.toString()}' instead.`);
}
function __PRIVATE_validateQueryFilterConstraint(e, t) {
  if (!(t instanceof QueryFieldFilterConstraint || t instanceof QueryCompositeFilterConstraint)) throw new FirestoreError(v2.INVALID_ARGUMENT, `Function ${e}() requires AppliableConstraints created with a call to 'where(...)', 'or(...)', or 'and(...)'.`);
}
...
var QuerySnapshot = class {
  ...
  docChanges(e = {}) {
    ...
    if (t && this._snapshot.excludesMetadataChanges) throw new FirestoreError(v2.INVALID_ARGUMENT, "To include metadata changes with your document changes, you must also pass { includeMetadataChanges:true } to onSnapshot().");
...
var WriteBatch = class {
  ...
  _verifyNotCommitted() {
    if (this._committed) throw new FirestoreError(v2.FAILED_PRECONDITION, "A write batch can no longer be used after commit() has been called.");
  }
};
function __PRIVATE_validateReference(e, t) {
  if ((e = getModularInstance(e)).firestore !== t) throw new FirestoreError(v2.INVALID_ARGUMENT, "Provided document reference is from a different Firestore instance.");
  return e;
}
...
function runTransaction(e, t, n) {
  ...
  !function __PRIVATE_validateTransactionOptions(e2) {
    if (e2.maxAttempts < 1) throw new FirestoreError(v2.INVALID_ARGUMENT, "Max attempts must be at least 1");
  }(r2);
...
function __PRIVATE_tryGetString(e, t) {
  if ("string" != typeof e[t]) throw new FirestoreError(v2.INVALID_ARGUMENT, "Missing string value for: " + t);
  return e[t];
}
!function __PRIVATE_registerFirestore(e, t = true) {
  !function __PRIVATE_setSDKVersion(e2) {
    ...
    const i = e2.getProvider("app").getImmediate(), s = new Firestore(new __PRIVATE_FirebaseAuthCredentialsProvider(e2.getProvider("auth-internal")), new __PRIVATE_FirebaseAppCheckTokenProvider(e2.getProvider("app-check-internal")), function __PRIVATE_databaseIdFromApp(e3, t2) {
      if (!Object.prototype.hasOwnProperty.apply(e3.options, ["projectId"])) throw new FirestoreError(v2.INVALID_ARGUMENT, '"projectId" not provided in firebase.initializeApp.');
